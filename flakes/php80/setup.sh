...
nix --extra-experimental-features nix-command --extra-experimental-features flakes build $BASE_DIR
...
SERVICE_DEST="/etc/systemd/system/multi-user.target.wants/${PHP_FPM_SERVICE}"
sudo rm -f $SERVICE_DEST
sudo ln "${BASE_DIR}/${PHP_FPM_SERVICE}" $SERVICE_DEST
